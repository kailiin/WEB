...
$link = ft_connectdb();
$query = "SELECT article.id, article.name, article.quantity, article.image, article.price FROM `article` JOIN `articles_has_category` AS ac ON article.id = ac.article_id WHERE ac.category_id = 1";
